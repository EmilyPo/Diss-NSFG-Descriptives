...
```{r alter-functions, echo=FALSE, include=FALSE}
# Functions to reshape ego data into alters
###########################################################
# reshape_edgelist
###########################################################
...
```{r reshape-alters, echo=FALSE}
alter_vars <- c("ego", "active1", "active2", "active3", "len1", "len2", "len3",
                  "dfs1", "dls1", "dfs2", "dls2", "dfs3", "dls3",
                  "rel1", "rel2", "rel3", "page1", "page2", "page3", "prace1", "prace2", "prace3", 
                  "diff.sqrt.age1", "diff.sqrt.age2", "diff.sqrt.age3",
                  "once1", "once2", "once3")

alters <- nsfg1529 %>% rename(ego=CASEID) %>% select(alter_vars)

alters <- reshape_edgelist(alters, delete_empty = c('active', 'len'))

alters <- alters %>% filter(active==1) %>%
  rename(race = prace, edge_age_month = len, age=page) %>% 
  mutate(network = ifelse(once==1, "inst", "main")) %>%
  mutate(network2 = ifelse((rel==1|rel==2), "cohab", 
                          ifelse(rel==3, "pers", 
                                 ifelse(once==1, "inst", NA)))) %>%
  mutate(agecat = ifelse(age %in% 1:14, "14-", 
                                ...
                                ifelse(age %in% 25:29, "25-29", 
                                       ifelse(age %in% 30:34, "30-34", 
                                              ifelse(age %in% 35:39, "35-39", 
                                                     ifelse(age %in% 40:45, "40-44", 
                                                            ifelse(age %in% 45:94, "45+", 
                                                                   ifelse(age %in% 95:99, "missing", NA)))))))))) %>%
  mutate(edge_id = row_number()) 
```

## Age Dist
(what do we do with over-29s and under-15s??? delete?)

```{r alter-age-dist, echo=FALSE}

ages <- alters %>% count(agecat) 
ages %>% plot_ly(x=~agecat, y=~n, type="bar")
m_ages <- alters %>% filter(age > 90) %>% count(age)

datatable(m_ages[,2], rownames = c("Not Asked", "DK/ref/miss"), caption = "Missings", width = "70%")
...
```{r alter-race, echo=FALSE}
races <- alters %>% count(race) 
races <- races[c(1,2,5,6),]
races %>% plot_ly(x=~race, y=~n, type="bar")
m_races <- alters %>% filter(race %in% "n.ask" | race %in% "miss/ref/dk") %>% count(race)

datatable(m_races, caption = "Missings", width = "70%")
```
